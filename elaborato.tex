...
AT: fai un ulteriore sforzo e usa anche quella di Davies, così li stronchiamo definitivamente ;)}
...
\begin{itemize}
    ...
    as a common resource}): sono in essere processi e meccanismi per
permettere alla community di migliorare i \textit{dataset} e creare
nuove risorse basate su quanto già pubblicato.
\end{itemize}
...
rivela\footnote{Enfasi aggiunta}\todo{AT: manca enfasi latex?} che \textcquote{Salani}{FER non solo
...
della circolazione ferroviaria, \textbf{convertirli in Open Data}\todo{AT: tecnicamente non è che li ``converti in opendata'' (anche perché non li stai rilasciando), casomai li ``rendi fruibili per una futura liberazione'', o simili} e
...
informazioni in \textbf{modo incrementale}\todo{AT: i.e., campionando con una frequenza ``opportuna''}.
Le particolarità e i casi eccezionali, molto frequenti nelle API considerate, devono essere
gestiti opportunamente\todo{AT: sinonimo più esplicito, magari ``integrando/incrociando dati e/o calcolando informazione'')}.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \EndWhile
    \end{algorithmic}
\end{algorithm}\todo{AT: caption e indica che formalismo stai usando, e cita il tutto nel testo}
...
codice ogni ora\todo{AT: da qualche parte spieghi il perché della scelta del periodo di campionamento?}.
La continuità e la persistenza sono ottenute grazie
...
Le API di Viaggiatreno (vedi \S~\ref{viaggiatreno}) non dispongono
di una documentazione ufficiale e non sono conformi alla architettura
