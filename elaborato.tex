...
\textit{reale} performance\todo{AT: i termini inglesi andrebbero in corsivo a meno che non siano accettati in dizionario italiano} del \textbf{trasporto pubblico
...
\todo{AT: c'è un salto, forse non andava commentato inizio frase?}
...
Stato.\todo{AT: beh, non tanto "curiosamente", in fondo gliene hanno fatte di tutti i colori a Italo (googla un po') pur di farlo fallire, oppure metti il curiosamente in corsivo ;)}



\todo{AT: da qualche parte fare un piccolo excursus su "alla ricerca delle API perdute"? sui tentativi (sia tecnici che legislativi) di estrarre info "sensate" (4-5 stelle Berners-Lee)}

\todo{AT: btw da qualche parte farei un microexcursus sulle classificazioni opendata (lo puoi riassumere dal secondo volume di CDT che ti posso mandare, è in fase di review ora)}
...
\textbf{Aldo Salani}\todo{AT: ti ha dato esplicito permesso di citarlo?}, responsabile dell'Area Commerciale FER e
...
rivela\footnote{Enfasi aggiunta} che \textcquote{Salani}{FER non solo
    ...
    Regioni} non sono così trasparenti?  \textcquote{Salani}{Intanto
    il modello FER è attualmente difficilmente applicabile alle altre
...
nei contratti di servizio.\todo{AT: "illustrati qui di seguito"}
