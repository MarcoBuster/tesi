...
\section{Panorama giuridico-legislativo}
...
Non è affatto scontato che i dati del trasporto \textit{pubblico} ---
...
senza il consenso di Trenitalia.  \textbf{Interpretando alla lettera}
le citate note legali, infatti, un viaggiatore che aiuta un turista a
Roma Termini comunicandogli l'orario del prossimo treno per Milano
Centrale sta \textit{distribuendo} senza autorizzazione un
\textit{contenuto} del Portale a terzi.  Ancora, l'autore di questa
Tesi di Laurea rischia di essere \textit{perseguito} per la sola
inclusione --- senza autorizzazione --- di \textit{link} al sito
\texttt{www.ViaggiaTreno.it}.  Non solo Trenitalia non pubblica i
dati, ma \textbf{attacca legalmente} chi cerca di reperirli o
diffonderli.  Può farlo?

\subsection{La vicenda Trenìt}
...
\textbf{prezzi} dei biglietti e permette di comparare le offerte
commerciali delle imprese ferroviare in libero mercato.

Il 19 luglio 2019, Trenìt è costretta a \textbf{sospendere il
...
\begin{quote}
    \textcquote[Nota di Trenìt]{TrenitSospensione}{Purtroppo dobbiamo
        sospendere temporaneamente il servizio, perché Trenitalia non
        vuole che pubblichiamo i dati (pubblici) relativi ai suoi
        treni e quindi ci ha fatto causa, ritenendo che l’uso che
        facciamo dei suoi dati sia illegittimo in quanto non
        autorizzato. Noi, invece, siamo convinti che il nostro operato
        sia pienamente lecito e non arrechi alcun danno agli operatori
        ferroviari. Al contrario, pensiamo che il nostro uso dei dati
        pubblici relativi al trasporto ferroviario arrechi un
        vantaggio agli operatori (visto che promuove i loro servizi).}
\end{quote}
...
dallo scraping e una \textbf{comparazione imparziale} delle offerte
commerciali.  Di seguito è riportato il suo commento a \textit{Wired}.
...
Trenitalia \textbf{non si è dimostrata aperta} alla condivisione delle
informazioni.  I termini di servizio di Viaggiatreno
\cite{ViaggiatrenoToS} non permettono nessun uso diverso da quello
...
Non sono risultate evidenze di \textbf{distorsioni} del confronto tra
le offerte di Trenitalia e della sua concorrenza.  È possibile
rilevare, invece, l'assenza di offerta della concorrenza su
Viaggiatreno e sull'app di Trenitalia.
...
È stato riconosciuto in giudizio che \textcquote{TrenitSentenza}{il
    ...
    server}.  Inoltre, se Trenìt non esistesse è ragionevole supporre
che la \textit{domanda di informazione} dovrebbe essere comunque
soddisfatta da Trenitalia stessa, in quanto si trova in condizione di
monopolio per l'\textbf{accesso delle informazioni}.
...
contenuto della banca dati.  L'art.\ 102 ter LDA\footnote{Legge sul
    diritto d'autore} recita infatti che
\textquote{\textit{\textbf{non sono soggette all'autorizzazione} del
        costitutore della banca dati \textbf{messa per qualsiasi
            motivo a disposizione del pubblico} le attività di
        estrazione o reimpiego di \textbf{parti non sostanziali}
        valutate in termini qualitativi e quantitativi del contenuto
        della banca dati per qualsivoglia fine effettuate dall'utente
        legittimo}}.  Trenitalia non ha presentato ulteriori ricorsi.

\subsubsection{Osservazioni}

Carlo Piana, noto avvocato italiano attivo nel campo del software
libero, ha pubblicato delle osservazioni alcuni punti della sentenza
nel suo articolo ``\textit{Scraping di dati pubblici per non perdere
    il treno della concorrenza}'' \cite{TrenitPiana} per
TechEconomy2030.  \todo{Marco: Spiegare ragioni...}
