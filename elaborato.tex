...
%			______________                                                %
%                                                                         %
%           Ultima revisione: 30 settembre 2023                              %
...
contenuto della banca dati.  L'art.\@ 102 ter LDA sancisce infatti che
...
Libero\footnote{\label{sw_libero}Un Software si dice \textit{Libero}
    quando \textcquote[vedi][]{GnuSwLibero}{gli utenti hanno la
        \textit{libertà} di eseguire, copiare, distribuire, studiare,
        modificare e migliorare il software}.},
% \todo{AT: footnote o altro che racconti in due parole cosa si
% intende, dato che poi dici che il tuo è GPL (ma manca un ref alla
% licenza!  metterlo)}
ha pubblicato delle osservazioni su alcuni punti della sentenza nel
suo articolo ``\textit{Scraping di dati pubblici per non perdere il
    treno della concorrenza}''~\cite{TrenitPiana} per TechEconomy2030.
...
\begin{sidewaysfigure}[p]
    ...
    \caption{Indice di puntualità delle direttrici Trenord per il
        2022~\cite{FoiaLomb,StatJup}}
    \label{figure:lomb_heatmap}
\end{sidewaysfigure}

In \figurename~\ref{figure:lomb_heatmap} sono mostrati i dati
trasmessi da Regione Lombardia riguardo la
\textbf{puntualità}\footnote{Come già detto, per \textit{indice di
        puntualità} si intende la percentuale di treni arrivata a
    destino con un ritardo di al più 5 minuti, escludendo le
    perturbazioni dovute a \textit{cause di forza maggiore}, scioperi
    e lavori programmati} delle corse Trenord nel 2022.  La tabella
originale è stata rielaborata in modalità \textit{heatmap} per
...
\begin{sidewaysfigure}[p]
    ...
    \caption{Boxplot dell'indice mensile di puntualità delle
        direttrici Trenord per il 2022~\cite{FoiaLomb,StatJup}}
...
\begin{figure}[h] \centering
    \begin{tikzpicture}[ round/.style={circle, draw=gray, minimum
            ...
            inner sep=2.5mm, fill=lightgray!50}, ]

        \node[bigrect] (scraper) at (0,0) {\textbf{\textit{Scraper}}};

        \node[rect] (viaggiatreno) [above right=12.5mm and 8.5cm of
        scraper.west] {API Viaggiatreno e Trenord};

        \node[bigrect] (extractor) [below=20mm of scraper]
        {\textbf{\textit{Extractor}}};

        \node[rect] (pickle) [above right=17.5mm and 8.5cm of
        extractor.west, anchor=west] {File \texttt{.pickle}
            incrementali};

        n \node[bigrect] (analyzer) [below=20mm of extractor]
        {\textbf{\textit{Analyzer}}}; \node[rect] (csv) [above
        right=17.5mm and 8.5cm of analyzer.west, anchor=west] {Dataset
            \texttt{.csv} giornalieri};
    
        \node[rect] (results) [below right=12.5mm and 8.5cm of
        analyzer.west] {Statistiche, visualizzazioni, \dots};

        \draw[blue, <-, thick] (scraper.150) to [bend left=90,
        looseness=1] node[blue, sloped, midway, fill=white] {Ogni ora}
        (scraper.30);
        ...
        \draw[blue, <-, thick] (extractor.150) to [bend left=90,
        looseness=1] node[blue, sloped, midway, fill=white] {Ogni
            giorno} (extractor.30);
...
l'\textbf{andamento} è prevalentemente \textbf{lineare} ($\rho =
0.73$,~\cite[K]{StatJup}).
...
Python, sono \textbf{liberi} (nel senso di \textit{Software
    Libero}\footnote{Vedi nota~\ref{sw_libero} in
    \S~\ref{caso_trenit}}
% \todo{AT: aggiungi un "vedi sopra" e vedi mia altra nota zona Carlo
% Piana})
e sono utilizzabili da linea di comando.  Per la prima volta è
disponibile una \textit{suite} di strumenti utile a
\textbf{scaricare}, \textbf{convertire} e \textbf{analizzare} in modo
indipendente i dati della circolazione ferroviaria italiana.
